...
select distinct t.town_id,f.street 
 ...
 where street is not null and char_length(street) >3
 order by 1,2;
...
select distinct a.firm_id,s.street_id,f.building,f.bletter,f.office,f.oletter 
  from artex_all._firm f
 ...
 group by a.firm_id,s.street_id,f.building,f.bletter,ifnull(f.office,a.firm_id),f.oletter 
 order by 2,3,1
...
select distinct d.address_id,1,t.code,f.phone,f.div_name 
  from artex_all._firm f
 ...
 where d.address_id is not null and length(phone) in (3,6,10) 
 group by a.firm_id,d.address_id,t.code,f.phone;
